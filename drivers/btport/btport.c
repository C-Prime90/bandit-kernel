...
static ssize_t btport_read(struct file *file, char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if (btlinux_port->minor == 1) {
		if (!ctxt->ch) {
			...
			if (req) {
				...
				printk(KERN_INFO "\n");
				copy_to_user(buffer, req->buf, req->length);
				bt_put_req(ctxt, &ctxt->tx_idle, req);
				ctxt->write_avail = 0;
				return req->length;
			}
			...
			if (nRead > 0) {
				...
				copy_to_user(buffer, pBuf, nRead);
	...
	if (!tx_circ_empty(btlinux_port)) {
		dbg("request read %d bytes from circ buf (%d) MAX = %d", count, bytesInCircBuf, UART_XMIT_SIZE);
 ...
 *
 ****************************************************************************/
static int send_to_dmrouter(const char __user *user_buffer,  int xfer)
{
	struct bt_request *req;
	struct bt_diag_context *ctxt = &_context;
	UINT8 *cmd;
	...
	if (!req) {
		...
		if (copy_from_user(req->buf, user_buffer, xfer)) {
			err("BTLINUXPORT %s: copy from user error", __func__);
			bt_put_req(ctxt, &ctxt->rx_idle, req);
			return -EINVAL;
		} else {
...
unsigned int btport_poll(struct file *file, struct poll_table_struct *p_pt)
{
	...
	if (!btlinux_port->port.info)
	    return mask;
	xmit = &btlinux_port->port.info->xmit;
...
module_param_call(bt_dun_enabled, NULL, show_bt_dun_enabled, 0, 0664);
